function zexo_trigger_deploy() {
   swal({
        title: "确认重新部署",
        text: "这将触发 Cloudflare Pages 重新构建您的博客站点。",
        icon: "warning",
        buttons: ["取消", "确认部署"],
        dangerMode: true
    })
    .then((value) => {
        ...
        if (value) {
            ...
            var ajax = ajaxObject();
            ajax.open("post", '/zexo/admin/api/trigger-deploy', true);
            ...
            ajax.onreadystatechange = function () {
                if (ajax.readyState == 4) {
                    ...
                    if (ajax.status == 200) {
                        sweetAlert("成功", "已成功触发重新部署！", "success");
                    } else {
                        sweetAlert("糟糕", "触发部署失败！", "error");
                    }
                }
            };
...
var ajax = ajaxObject();
    ajax.open( "post" , '/zexo/admin/api/inputtalk' , true );
